...
5. Upload the disk image to azure with `./hosts/x86_64-linux/jellyfin-nixos-on-azure/upload-image.sh --resource-group kosh-jellyfin --image-name jellyfin-nixos`.
6. Create the VM from the uploaded image with `./hosts/x86_64-linux/jellyfin-nixos-on-azure/boot-vm.sh --resource-group kosh-jellyfin --image jellyfin-nixos --vm-name jellyfin-nixos`.